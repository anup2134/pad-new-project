...
function App() {
    ...
    // Generate TTS with highlighting using Web Speech API
    const handleSpeak = () => {
        ...
        utterance.onstart = () => {
            setIsPlaying(true);
            setCurrentWordIndex(-1);
            setWordTimings([1]); // Dummy to trigger word-wrapped rendering
        };

        utterance.onboundary = (event) => {
            if (event.name === 'word') {
                // Find word index by counting words up to charIndex
                const textBefore = content.substring(0, event.charIndex);
                // Use the same regex as in renderHighlightedText for consistency
                const wordsBefore = textBefore.split(/(\s+)/).filter(w => !/^\s+$/.test(w) && w.length > 0);
                setCurrentWordIndex(wordsBefore.length);

                // Update progress
                setProgress((event.charIndex / content.length) * 100);
            }
        };
    ...
    const renderHighlightedText = () => {
        ...
        if (wordTimings.length === 0) {
            if (settings.focusMode) {
                ...
                return <div className="text-display">{content}</div>;
            }
        }
